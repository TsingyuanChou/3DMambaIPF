...
class DenoiseNet(pl.LightningModule):
    ...
    def get_supervised_loss_nn(self, pcl_noisy, pcl_clean, pcl_seeds, pcl_std):
        ...
        for i in range(self.num_modules):
            ...
            device_dist = dist.device
            clean_nbs_views = torch.Tensor(render_views(clean_nbs.reshape(B, -1, 3), 1).numpy()).to(device_dist)
            pred_disp_views = torch.Tensor(render_views(pred_disp.reshape(B, -1, 3), 1).numpy()).to(device_dist)
            loss_view = torch.mean(torch.abs(clean_nbs_views - pred_disp_views))
            
            # print((seed_weights * dist).sum(dim=-1).mean(dim=-1), loss_view)
            losses[i] = (seed_weights * dist).sum(dim=-1).mean(dim=-1) + 0.01 * loss_view
